...
fn separar_datos_delete(consulta_sql:String) -> Result<(String, Vec<String>),&'static str>{
...
fn borrar_lineas_csv(ruta_csv:String,header:Vec<String>,clave:Vec<String>)-> io::Result<()>{
    let archivo = File::open(&ruta_csv)?;
    let lector = BufReader::new(archivo);
    let archivo_temporal = "auxiliar.csv";
    let mut archivo_tem = File::create(archivo_temporal)?;
...
fn eliminar_datos(consulta_sql: String, ruta_del_archivo: String){


    let (nombre_del_csv,clave) =  match separar_datos_delete(consulta_sql) {
    ...
    let _ = borrar_lineas_csv(ruta_csv, header, clave);
